...
#define TYPE_IS_UINT_SIZET_(t, o)              \
    (                                          \
        (TYPE_IS_UNSIGNED_INT(t)) &&           \
        (TYPE_UNSIGNED_INT_MAX_(t) o SIZE_MAX) \
    )
#define TYPE_IS_UINT_MAX_SIZET(t) \
    TYPE_IS_UINT_SIZET_(t, <=)
...
#define UINT_AS_UCHAR_(m, x)               \
    ({                                     \
        STATIC(TYPEOF_IS_UNSIGNED_INT(x)); \
        m((x) <= UCHAR_MAX);               \
        (uchar_t) (x);                     \
    })
#define VERIFY_UINT_AS_UCHAR(x) \
    UINT_AS_UCHAR_(VERIFY, x)
#define UINT_AS_UCHAR(x) \
    UINT_AS_UCHAR_(ASSERT, x)
